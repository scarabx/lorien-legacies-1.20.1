...
public class TelekinesisEffect extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        ...
        if (entity instanceof ServerPlayerEntity player) {
            ...
            if (player.hasStatusEffect(TOGGLE_TELEKINESIS_MOVE) && !player.hasStatusEffect(TIRED) && !player.hasStatusEffect(ACTIVE_LEGACY_INHIBITION))
    ...
    public static void push(ServerPlayerEntity player) {
        ...
        if (!player.getWorld().isClient() && player.hasStatusEffect(ModEffects.TELEKINESIS) && player.hasStatusEffect(TOGGLE_TELEKINESIS_PUSH) && !player.hasStatusEffect(TIRED) && !player.hasStatusEffect(ACTIVE_LEGACY_INHIBITION)) {
    ...
    public static void pull(ServerPlayerEntity player) {
        ...
        if (!player.getWorld().isClient() && player.hasStatusEffect(ModEffects.TELEKINESIS) && player.hasStatusEffect(TOGGLE_TELEKINESIS_PULL) && !player.hasStatusEffect(TIRED) && !player.hasStatusEffect(ACTIVE_LEGACY_INHIBITION)) {
    ...
    public static void move(ServerPlayerEntity player) {
        ...
        if (!player.getWorld().isClient() && player.hasStatusEffect(ModEffects.TELEKINESIS) && player.hasStatusEffect(TOGGLE_TELEKINESIS_MOVE) && !player.hasStatusEffect(TIRED) && !player.hasStatusEffect(ACTIVE_LEGACY_INHIBITION)) {
            ...
            // positive = looking down, negative = up

            double pitch = player.getPitch();

            // maps -90..90 to -2..2

            double gripAdjustment = pitch / 90.0 * 2.0;
            ...
            if (target != null) {
                ...
                Vec3d moveVec = targetPos.subtract(currentPos).multiply(0.4);
    ...
    public static void deflect(ServerPlayerEntity player) {
        ...
        if (!player.getWorld().isClient() && player.hasStatusEffect(ModEffects.TELEKINESIS) && player.hasStatusEffect(TOGGLE_TELEKINESIS_DEFLECT) && !player.hasStatusEffect(TIRED) && !player.hasStatusEffect(ACTIVE_LEGACY_INHIBITION) && !player.hasStatusEffect(DEFLECT_COOLDOWN) && !player.hasStatusEffect(TOGGLE_IMPENETRABLE_SKIN) && !player.hasStatusEffect(TOGGLE_INTANGIBILITY)) {
