...
public class IceballItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            IceballProjectileEntity iceballProjectile = new IceballProjectileEntity(user, world);
            iceballProjectile.setItem(itemStack);
            iceballProjectile.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 0f);
